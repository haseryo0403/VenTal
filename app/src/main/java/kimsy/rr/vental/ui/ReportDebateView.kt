...
@Composable
fun ReportDebateView(
    viewModel: ReportDebateViewModel = hiltViewModel(),
    toDebateView: () -> Unit
) {
    ...
    when(reportState.status) {
        ...
        Status.FAILURE -> {
            Toast.makeText(LocalContext.current, stringResource(id = R.string.report_failure), Toast.LENGTH_LONG).show()
            viewModel.resetState()
        }
        else -> {}
    }
    LazyColumn(
        ...
        item {
            Button(
                ...
                onClick = { viewModel.reportDebate(selectedOption) }
            ) {
